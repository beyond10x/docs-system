...
try {
    if (command === 'validate') {
        if (args.length === 0)
            throw new Error('usage: b10x-docs validate <manifest-or-change>...');
        for (const file of args) {
            const document = await readDocument(file);
            const detail = document.schema.startsWith('b10x-docs/') ? `${document.surfaces.length} surface(s)` : 'ecosystem change';
        ...
        const manifests = documents.filter((document) => document.schema.startsWith('b10x-docs/'));
        const changes = documents.filter((document) => document.schema === 'b10x-change/v1');
        const registry = buildRegistry(manifests);
