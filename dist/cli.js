...
try {
    if (command === 'validate') {
        if (args.length === 0)
            throw new Error('usage: b10x-docs validate <manifest-or-change-or-lock-or-redirect-map>...');
        for (const file of args) {
            const document = await readDocument(file);
            let detail;
            if (document.schema.startsWith('b10x-docs/'))
                detail = `${document.surfaces.length} surface(s)`;
            else if (document.schema.startsWith('b10x-change/'))
                detail = 'ecosystem change';
            else if (document.schema === 'b10x-experiences/v1')
                detail = `${document.experiences.length} experience(s), ${document.artifacts.length} artifact(s)`;
            else if (document.schema === 'b10x-doc-page/v1')
                detail = 'document page metadata';
            else if (document.schema === 'b10x-sources/v1')
                detail = `${document.sources.length} locked source(s)`;
            else if (document.schema === 'b10x-redirects/v1')
                detail = `${document.redirects.length} compatibility route(s)`;
            else
        ...
        if (args.length === 0)
            throw new Error('usage: b10x-docs validate-page <markdown-or-mdx>...');
        for (const file of args) {
            const metadata = await readDocumentPageMetadata(file);
            if (!metadata)
                throw new Error(`${file} has no top-level b10x frontmatter metadata`);
        ...
        let rest = args;
        const registryOption = takeOption(rest, '--registry-out');
        rest = registryOption.rest;
        const ledgerOption = takeOption(rest, '--ledger-out');
        rest = ledgerOption.rest;
        const rssOption = takeOption(rest, '--rss-out');
        rest = rssOption.rest;
        const feedOption = takeOption(rest, '--json-feed-out');
        rest = feedOption.rest;
        const releaseOption = takeOption(rest, '--release-facts');
        rest = releaseOption.rest;
        ...
        let rest = args;
        const manifestOption = takeOption(rest, '--manifest');
        rest = manifestOption.rest;
        const rootOption = takeOption(rest, '--repository-root');
        rest = rootOption.rest;
        const indexOption = takeOption(rest, '--index-out');
        rest = indexOption.rest;
        const documentIndexOption = takeOption(rest, '--document-index-out');
        rest = documentIndexOption.rest;
        const outOption = takeOption(rest, '--out');
        rest = outOption.rest;
        ...
        if (manifest.schema !== 'b10x-docs/v3' && manifest.schema !== 'b10x-docs/v4' && manifest.schema !== 'b10x-docs/v5')
            throw new Error('b10x-docs collect requires a b10x-docs/v3, b10x-docs/v4 or b10x-docs/v5 manifest');
        ...
        let rest = args;
        const mapOption = takeOption(rest, '--map');
        rest = mapOption.rest;
        const outOption = takeOption(rest, '--out');
        rest = outOption.rest;
        const aliasOption = takeOption(rest, '--alias-root');
        rest = aliasOption.rest;
        ...
        const manifestOption = takeOption(rest, '--manifest');
        rest = manifestOption.rest;
        const fileOption = takeOption(rest, '--file');
        rest = fileOption.rest;
        const checkOption = takeFlag(rest, '--check');
        rest = checkOption.rest;
...
function isManifest(document) {
    return document.schema.startsWith('b10x-docs/');
}
function isChange(document) {
