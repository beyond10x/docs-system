...
const canonicalLanguages = new Set(PRISM_LANGUAGES);
/**
 ...
 */
export function normalizeMarkdownFenceLanguage(language) {
 ...
 */
export function markdownFenceProblems(fence) {
    const problems = [];
    ...
    if (marker) {
        ...
        if (!closePattern.test(fence.raw))
            problems.push('fenced code block is not closed');
    }
    return problems;
}
...
export function normalizePassiveMarkdown(source) {
    ...
    for (const original of stripManagedRanges(source).split(/\r?\n/)) {
        ...
        if (heading)
            output.push(`<a id=${JSON.stringify(heading[3])}></a>`, `${heading[1]} ${heading[2]}`);
        else if (line || !htmlComment)
            output.push(line);
    }
    return output.join('\n');
}
...
export function escapePlaceholderTags(line) {
    ...
    while (cursor < line.length) {
        ...
        if (opening < 0)
            return output + escapeProsePlaceholderTags(line.slice(cursor));
        let runEnd = opening;
        while (line[runEnd] === '`')
            runEnd += 1;
...
function escapeProsePlaceholderTags(value) {
    return value.replace(/<([a-z][a-z0-9.-]*)>/g, (match, tag) => (passiveHtmlElements.has(tag) ? match : `&lt;${tag}&gt;`));
}
/** Canonicalize only the language token of an opening fence line; `console` stays visible. */
export function normalizeFenceLine(line) {
...
export function stripManagedRanges(source) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (managedRange) {
            if (!marker)
                continue;
            if (marker[2].toLowerCase() === 'start')
                throw new ManagedRangeError(`nested managed documentation range ${marker[1]}`, index + 1);
        ...
        if (marker[2].toLowerCase() === 'end')
            throw new ManagedRangeError(`managed documentation range ${marker[1]} ends without a start`, index + 1);
        managedRange = marker[1].toLowerCase();
        managedStart = index + 1;
    }
    if (managedRange)
        throw new ManagedRangeError(`managed documentation range ${managedRange} is not closed`, managedStart);
    return output.join('\n');
}
