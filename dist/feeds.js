...
export async function writeRss(out, ledger, options = {}) {
...
export async function writeJsonFeed(out, ledger, options = {}) {
    const scope = options.scope ?? 'all';
    const homePageUrl = options.homePageUrl ?? defaultHomePage(scope);
    const feed = {
...
export function selectFeedEntries(ledger, scope) {
    if (scope === 'all')
...
function defaultHomePage(scope) { return scope === 'releases' ? `${origin}/releases/` : `${origin}/changes/`; }
function defaultTitle(scope) { return scope === 'releases' ? 'beyond10x releases' : scope === 'impact' ? 'beyond10x impactful changes' : 'beyond10x ecosystem changes'; }
