...
/**
 * Navigation as any manifest revision declares it. v3 and v4 allow only the enum sidebar; v5 also
 * allows a declared tree, a landing document and menu-withheld documents. `sidebar` is present in
 * every manifest and absent only where the public registry leaves out a v5 sidebar tree.
 */
export interface SourceNavigation {
  label?: string;
  group?: string;
  order?: number;
  sidebar?: 'autogenerated' | 'flat' | SidebarCategory[];
  /** The published document served at the surface's route base instead of the generated profile. */
  landing?: string;
  /** Published documents that stay routable but are left out of the menu. */
  menuWithheld?: string[];
}

/** Navigation as v3 and v4 declare it. */
export interface SourceNavigationV3 extends Omit<SourceNavigation, 'sidebar' | 'landing' | 'menuWithheld'> {
...
/** A sidebar category; each leaf is a published document path relative to `source.root`. */
export interface SidebarCategory {
  label: string;
  collapsed?: boolean;
  items: Array<string | SidebarCategory>;
}

/** Navigation as v5 declares it: `sidebar` is required and may be a tree. */
export interface SourceNavigationV5 extends SourceNavigation {
  sidebar: 'autogenerated' | 'flat' | SidebarCategory[];
}

export interface SurfaceSourceV5 extends Omit<SurfaceSource, 'navigation'> {
  navigation?: SourceNavigationV5;
}

export interface DocumentationSurfaceV5 extends Omit<DocumentationSurfaceV4, 'source'> {
  source: SurfaceSourceV5;
  /** Built files, relative to `source.root`, that the repository's own credential-free workflow produces. */
  builtAssets?: string[];
}

export interface DocumentationManifestV5 extends Omit<DocumentationManifestV4, 'schema' | 'surfaces'> {
  schema: 'b10x-docs/v5';
  surfaces: DocumentationSurfaceV5[];
}

/** v5 navigation as the public registry carries it: the v4 fields only; a declared sidebar tree is omitted. */
export interface RegistryNavigationV5 extends Omit<SourceNavigation, 'sidebar' | 'landing' | 'menuWithheld'> {
  sidebar?: SourceNavigationV3['sidebar'];
}

/** A v5 surface in the registry's v4 shape: no built assets, landing, withheld documents or sidebar tree. */
export interface RegistrySurfaceV5 extends Omit<DocumentationSurfaceV5, 'source' | 'builtAssets'> {
  source: Omit<SurfaceSourceV5, 'navigation'> & {navigation?: RegistryNavigationV5};
}
