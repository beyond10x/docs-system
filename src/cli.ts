...
try {
  if (command === 'validate') {
    if (args.length === 0) throw new Error('usage: b10x-docs validate <manifest>...');
    ...
    if (marker < 0 || !args[marker + 1]) throw new Error('usage: b10x-docs registry --out <file> <manifest>...');
    const out = args[marker + 1];
    const files = args.filter((_, index) => index !== marker && index !== marker + 1);
    if (files.length === 0) throw new Error('registry requires at least one manifest');
