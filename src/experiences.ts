...
const accessRank: Record<Access, number> = {
  public: 0,
  'account-required': 1,
  'approval-required': 2,
  private: 3,
...
const actionableSupport = new Set<Support>(['supported', 'preview', 'experimental']);

/** Return the least permissive access value. `unspecified` is compatibility-only and safest. */
export function mostRestrictiveAccess(...values: Access[]): Access {
  if (values.length === 0) throw new Error('mostRestrictiveAccess requires at least one access value');
  return values.reduce((most, value) => accessRank[value] > accessRank[most] ? value : most);
}

export function validateExperienceCatalogSemantics(catalog: ExperienceCatalog, context = 'experience catalog'): void {
  const artifacts = new Set<string>();
  for (const artifact of catalog.artifacts) {
    if (artifacts.has(artifact.id)) throw new Error(`${context} contains duplicate artifact ${artifact.id}`);
    artifacts.add(artifact.id);
  }

  const experiences = new Set<string>();
  for (const experience of catalog.experiences) {
    if (experiences.has(experience.id)) throw new Error(`${context} contains duplicate experience ${experience.id}`);
    experiences.add(experience.id);
    const paths = new Set<string>();
    for (const adoptionPath of experience.adoptionPaths) {
      if (paths.has(adoptionPath.id)) throw new Error(`${context} experience ${experience.id} contains duplicate adoption path ${adoptionPath.id}`);
...
export function evaluateAdoptionPath(path: AdoptionPath, artifactById: ReadonlyMap<string, ExperienceArtifact>): EvaluatedAdoptionPath {
  const artifacts = path.artifactIds.map((id) => {
    const artifact = artifactById.get(id);
    if (!artifact) throw new Error(`adoption path ${path.id} requires unknown artifact ${id}`);
    return artifact;
  });
  ...
  const blockers: AdoptionPathBlocker[] = [];
  if (!actionableSupport.has(path.support)) blockers.push('non-actionable-support');
  if (unavailable.length > 0) blockers.push('unavailable-artifact');
  if (effectiveAccess === 'private') blockers.push('private-access');
  if (!path.url) blockers.push('missing-url');
...
export function evaluateExperienceCatalog(catalog: ExperienceCatalog): EvaluatedDocumentationExperience[] {
 ...
 */
export function normalizeManifestExperiences(
  ...
  return manifest.surfaces.map((surface) => ({
    ...
    experiences: surface.experienceIds.map((id) => evaluated.get(id) as EvaluatedDocumentationExperience),
  }));
}
...
export function validateManifestExperienceReferences(manifest: DocumentationManifestV4, catalog: ExperienceCatalog): void {
  ...
  for (const surface of manifest.surfaces) {
    ...
    for (const id of surface.experienceIds) {
      if (!experienceIds.has(id)) throw new Error(`${context} references unknown experience ${id}`);
    }
    for (const id of surface.documentDefaults.experienceIds) {
      if (!surface.experienceIds.includes(id)) throw new Error(`${context} document default experience ${id} is not declared by the surface`);
    }
  }
}
...
function compatibilityExperience(surface: AnyDocumentationSurface, journey: Journey): EvaluatedDocumentationExperience {
...
function compatibilityAdoptionPath(surface: AnyDocumentationSurface, adoption?: AdoptionAction): EvaluatedAdoptionPath {
...
function compatibilityExperienceId(journey: Journey): string {
  return `legacy-${journey}`;
}

function uniqueJourneys(journeys: Journey[] | undefined): Journey[] {
...
function explainBlockers(
  support: Support,
  access: Access,
  unavailable: ExperienceArtifact[],
  hasUrl: boolean,
): string | undefined {
  const reasons: string[] = [];
  if (!actionableSupport.has(support)) reasons.push(`Support status is ${support}.`);
  if (unavailable.length > 0) {
    reasons.push(`Required artifacts are not available: ${unavailable.map((artifact) => `${artifact.id} (${artifact.availability})`).join(', ')}.`);
  }
  if (access === 'private') reasons.push('Effective access is private.');
  if (access === 'unspecified') reasons.push('Access is unspecified.');
  if (!hasUrl) reasons.push('No adoption URL is declared.');
