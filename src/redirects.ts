...
export async function writeRedirectMap(outputRoot: string, map: RedirectMap, options: RedirectWriteOptions = {}): Promise<void> {
  ...
  for (const redirect of map.redirects) {
    ...
    if (destinations.has(relativeOutput)) throw new Error(`duplicate compatibility output ${relativeOutput}`);
    ...
    if (redirect.type === 'html') {
      ...
      if (!stat.isFile() || stat.isSymbolicLink()) throw new Error(`${redirect.source} is not a regular alias source file`);
      await fs.copyFile(source, destination);
    }
  }
}

export function renderRedirectHtml(origin: string, redirect: HtmlRedirect): string {
...
export function outputPathForRoute(route: string, htmlRoute = false): string {
  const value = route.replace(/^\/+/, '');
  if (!value) return 'index.html';
  const normalized = safeRelative(value.replace(/\/+$/, ''), 'compatibility route');
  if (htmlRoute && !path.posix.extname(normalized)) return `${normalized}/index.html`;
  if (htmlRoute && route.endsWith('/')) return `${normalized}/index.html`;
  return normalized;
}

function safeRelative(value: string, context: string): string {
  if (!value || value.includes('\\') || path.posix.isAbsolute(value)) throw new Error(`${context} must be a non-empty relative path`);
  const normalized = path.posix.normalize(value);
  if (normalized === '..' || normalized.startsWith('../')) throw new Error(`${context} escapes its root`);
  return normalized;
}

function ensureInside(root: string, candidate: string, context: string): void {
  const relative = path.relative(root, candidate);
  if (relative === '..' || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)) throw new Error(`${context} escapes its output root`);
}
