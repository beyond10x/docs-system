...
export async function parseDocumentPageMetadata(
  source: string,
  context = 'document page',
): Promise<DocumentPageMetadata | undefined> {
  const frontmatter = parseFrontmatter(source, context);
  if (!frontmatter || !Object.prototype.hasOwnProperty.call(frontmatter, 'b10x')) return undefined;
  return validateDocumentPageMetadata(frontmatter.b10x, `${context} b10x frontmatter`);
}

export async function readDocumentPageMetadata(file: string | URL): Promise<DocumentPageMetadata | undefined> {
...
export async function resolveDocumentPageMetadata(
...
export function effectiveDocumentPageMetadata(
  manifest: DocumentationManifest,
  surfaceId: string,
 ...
 */
export async function buildDocumentPageIndex(
  manifest: DocumentationManifest,
  collection: CollectionIndex,
  repositoryRoot: string,
): Promise<DocumentPageIndex> {
...
function parseFrontmatter(source: string, context: string): Record<string, unknown> | undefined {
  ...
  if (typeof value !== 'object' || Array.isArray(value)) throw new Error(`${context} frontmatter must be a mapping`);
