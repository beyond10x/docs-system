...
`b10x-docs/v5` is v4 plus declared navigation and built assets. `source.navigation.sidebar` is
either `autogenerated`/`flat` or a list of categories (`label`, optional `collapsed`, `items`) whose
leaves are document paths relative to `source.root`; `source.navigation.landing` names the document
served at the route base; `source.navigation.menuWithheld` lists routable documents kept out of the
menu; the surface-level `builtAssets` list names built files the repository's own credential-free
workflow commits. Validation refuses any sidebar leaf, landing or withheld path that
`source.documents` does not publish (or that lies under a directory the collector never enters),
any non-canonical or repeated path, a built asset that is a glob, a directory or under a directory
the collector never enters, and a withheld document that is also a sidebar leaf, naming the
path; invisible characters in a refused path are shown as `\uXXXX`. A withheld document may be the
landing document. The public registry carries a v5 surface in
the v4 shape, without the sidebar tree, landing, withheld documents or built assets.
